...
export class FileExplorerProvider implements vscode.TreeDataProvider<FileItem>, vscode.FileDecorationProvider {
  ...
  /**
   * Analyze a search query to determine its type (glob, path, literal)
   */
  private analyzeQuery(raw: string): { text: string; isPath: boolean; isGlob: boolean; isQuoted: boolean } {
    const q = (raw ?? '').trim();
    const isQuoted = q.startsWith('"') && q.endsWith('"');
    const unquoted = isQuoted ? q.slice(1, -1) : q;
    const isPath = /[\\/]/.test(unquoted);  // Check for both forward and backslash
    const isGlob = !isQuoted && /[*?{}]/.test(unquoted);  // Don't detect [] as we don't support it
    return { text: unquoted, isPath, isGlob, isQuoted };
  }

  // Set the search term and refresh the tree
  async setSearchTerm(searchTerm: string): Promise<void> {
    // Auto-detect search type from the query
    const query = this.analyzeQuery(searchTerm);
    
    console.log(`FileExplorer: Setting search term to "${searchTerm}", auto-detected: path=${query.isPath}, glob=${query.isGlob}`);

    const searchTermLower = query.text.toLowerCase();
    
    // Auto-detect settings
    const nextIsGlob = query.isGlob;
    // Include folders if: path search, glob with ** pattern, or explicit folder match
    const nextIncludeFolders = query.isPath || (query.isGlob && searchTerm.includes('**'));
